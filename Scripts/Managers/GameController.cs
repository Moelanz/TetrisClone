...
public class GameController : MonoBehaviour {
    ...
    [Range(0.02f,1f)]
	public float m_keyRepeatRateRotate = 0.15f;
    float m_timeToNextKeyRotate;

    [Header("Other settings")]
    // the time window we can rotate the shape
    [Range(0.02f, 1f)]
    public float m_fastDropInterval = 0.1f;
    float m_timeToFastDrop;
    ...
    void Start () 
	{
        ...
        m_timeToFastDrop = Time.time + m_fastDropInterval;
    ...
    void PlayerInput()
	{
        ...
        else if (m_swipeDirection == Direction.up /*|| ((Input.touchCount > 0 && Input.GetMouseButtonDown(0)) && m_swipeDirection == Direction.none)*/)
        {
            Rotate();
            ResetSwipeDirection();
        }
        ...
        else if(Input.GetButtonDown("FastDrop") && (Time.time > m_timeToFastDrop))
        {
            FastDrop();
        }
    }
    ...
    void LandShape ()
	{
		...
		{
            //Add combo to the score
            combo++;

            m_scoreManager.ScoreLines(m_gameBoard.m_completedRows, combo);
